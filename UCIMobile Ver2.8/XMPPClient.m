...
- (void)xmppStream:(XMPPStream *)stream didReceiveIQ:(XMPPIQ *)iq
{
	...
	if (vCardPhotoElement != nil) 
	{
		...
		NSData *imageData = [[base64DataString dataUsingEncoding:NSUTF8StringEncoding] base64Decoded];
		
		UIImage *avatarImage = [[UIImage alloc] initWithData:imageData];
		XMPPJID *jids = [iq from];
		XMPPUser *users = [roster objectForKey:jids];
		
		if (avatarImage!=nil) 
		{
			[users setPortrait:avatarImage];
		}
	}
	if([iq isRosterQuery])
	{
...
- (void)xmppStream:(XMPPStream *)stream didReceivePresence:(XMPPPresence *)presence
{
	...
	if ([photoHash length] > 0) 
	{   // in case when there's no photo hash
		XMPPJID *rosterJID = [presence from];
		BOOL requestPhoto = YES; // determine if you need to request new photo or nor
